...
//  parameter Modelica.Units.SI.Length d=0.5 "characteristic length of the leaf" annotation (Dialog(enable=not rb_constant));
...
//  Modelica.Units.SI.Velocity u = 0.04 "Local air velocity"   annotation (Dialog(group="Varying inputs",enable=not rb_constant));
//  Modelica.Units.SI.HeatFlux I_g = 300 "outside global solar radiation"  annotation (Dialog(group="Varying inputs"));
  ...
  Modelica.Units.SI.Temperature T_can=300 "Temperature of the canopy (port a)"
    annotation (Dialog(group="Varying inputs"));
//  Modelica.Units.SI.Temperature T_air=300
...
//  Real r_bH(unit="s/m") "boundary layer resistance to heat transfer of the leaves";
  ...
  Real S_rs;
  Modelica.Units.SI.SpecificEnergy DELTAH=2.45e6
